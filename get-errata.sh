...
egrep -m 1 -A 400 "^.h2.Red Hat Enterprise Linux.*(x86_64|AUS|(Extended Life Cycle Support))" $file_name | grep auth_= |gawk '{print $5}'|sort -u|grep "src.rpm\|x86_64\|noarch"|gawk -F">" '{print $1}'|sed 's/href=//g'|sed 's/\&amp;/\&/g'|gawk -F"[/?]" '{print "curl --output " $11 " " $0}' > $sh_filename
...
# ダウンロード元のチェックサムを集める。
egrep -m 1 -A 400 "^.h2.Red Hat Enterprise Linux.*(x86_64|AUS|(Extended Life Cycle Support))" $file_name | grep -B2 "src.rpm\|x86_64\|noarch" |grep SHA-256:|sort -u|gawk -F"<" '{print $2}'|gawk '{print $3}' >sha256sum.lst  
